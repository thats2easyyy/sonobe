...
The subscription path is experimental and off by default, and packaged builds hide its switch unless started with `SONOBE_CLAUDE_SUBSCRIPTION=1`, so no release offers it by accident ([ARCHITECTURE.md](ARCHITECTURE.md) §10). Keep that gate until Anthropic agrees. To try it on your own machine:
...
2. Run the desktop app from a checkout (`npm run desktop`), which always offers the switch, or start a packaged build with the gate open: `SONOBE_CLAUDE_SUBSCRIPTION=1 /Applications/Sonobe.app/Contents/MacOS/Sonobe`.
