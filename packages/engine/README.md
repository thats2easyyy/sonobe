...
| `inspect(address)` | `{ value, copies?, note? }`: what `getValue` reads, how many copies a layer drew last frame (or a component patch's instance ran), and a note when the value reads as nothing: the layer, or a layer it's inside, drew 0 copies (quoting its `empty_loop` warning), `#n` is past its copies, or why a loop is empty. |
