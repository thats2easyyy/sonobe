...
/** Pressure 0–1 when the pointer snapshot reports one (not yet in the contract), else 0. */
export function snapshotPressure(snap: object): number {
  const pressure = (snap as { pressure?: unknown }).pressure;
  return typeof pressure === "number" && Number.isFinite(pressure) ? clamp01(pressure) : 0;
}

export const interaction = withMutedBehavior(
  definePatch<InteractionState>("interaction", {
    ...
    evaluate(ctx) {
      ...
      ctx.output("force", live && snap.down ? snapshotPressure(snap) : 0);
