...
import { enumOr, releaseRef, safely } from "./shared.ts";
...
function finishRecording(s: CameraState, media: MediaCaptureServices | undefined): void {
  if (!s.recorderActive) return;
  s.recorderActive = false;
  if (media) trackRequest(s.requests, "recording", () => media.stopRecording(s.key));
}

function closeSession(s: CameraState, media: MediaCaptureServices | undefined): void {
...
export const cameraPatch = definePatch<CameraState>("camera", {
