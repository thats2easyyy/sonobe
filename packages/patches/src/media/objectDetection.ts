...
import type { PatchContext, PixelReading } from "@sonobe/engine";
...
export const objectDetectionPatch = definePatch<ObjectState>("objectDetection", {
  ...
  evaluate(ctx) {
    ...
    if (ctx.muted || !toBool(ctx.input("enabled")) || ref === null) {
      s.regions = [];
      s.lastFrame = null;
      s.error = "";
