...
import { layerOutputReader, layerTypeOf } from "./platform.ts";
import { isLayerRef, withMutedBehavior } from "./shared.ts";
...
export const videoInfoPatch = withMutedBehavior(
  definePatch("videoInfo", {
    evaluate(ctx) {
      ...
      if (ref !== null) {
        ...
        const read = layerOutputReader(ctx.services);
