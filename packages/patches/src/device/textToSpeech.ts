...
import type { RuntimeServices, SpeechOptions } from "@sonobe/engine";
import { clamp, definePatch, finiteOr, toText, warnOnce } from "../infra/index.ts";
import { describeError, isThenable } from "./shared.ts";
...
function stopUtterance(state: SpeechState, services: RuntimeServices): void {
  ...
  if (queue.owner === state.token) {
    queue.owner = null;
    services.platform.stopSpeaking?.();
  }
}

export const textToSpeechPatch = definePatch<SpeechState>("textToSpeech", {
