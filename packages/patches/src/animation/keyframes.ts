...
import { getInputCountRange, resolveTypeParam } from "@sonobe/core";
...
/** The catalog's `inputCountRange` (2…32, 3 by default). */
const RANGE = getInputCountRange(SPEC) ?? { min: 2, max: 32, defaultCount: 3 };

export const MIN_KEYFRAMES = RANGE.min;
export const MAX_KEYFRAMES = RANGE.max;
export const DEFAULT_KEYFRAMES = RANGE.defaultCount;

/** Number of keyframes for an inputCount: rounded and clamped to 2…32, default 3 when unset or not finite. */
export function keyframeCount(inputCount: number | undefined): number {
  const n = typeof inputCount === "number" && Number.isFinite(inputCount) ? Math.round(inputCount) : DEFAULT_KEYFRAMES;
...
export const keyframes = definePatch("keyframes", {
  ...
  evaluate(ctx) {
    ...
    const count = keyframeCount(ctx.inputCount);
    const stops: number[] = [];
    const values: number[][] = [];
    for (let n = 1; n <= count; n++) {
