...
export const LAYER_TYPES: LayerTypeSpec[] = [
  ...
  },
  {
    ...
    props: [
      ...COMMON_PROPS,
      prop("animation", "Animation", "json", null, "content", "Lottie asset."),
      prop("playing", "Play", "boolean", true, "content", "Plays while true."),
      prop("loop", "Loop", "boolean", true, "content", "Restart at the end."),
  ...
  },
  {
    ...
    name: "Shader",
    category: "advanced",
    summary: "A GPU fragment shader (GLSL ES 3.0, ShaderToy-style) that fills the layer. Child layers are available as a texture.",
    canHaveChildren: true,
    props: [
      ...COMMON_PROPS,
      prop("code", "Shader Code", "text", "void mainImage(out vec4 fragColor, in vec2 fragCoord) {\n  vec2 uv = fragCoord / iResolution.xy;\n  fragColor = vec4(uv, 0.5 + 0.5 * sin(iTime), 1.0);\n}\n", "content", "GLSL fragment code defining mainImage(out vec4, in vec2). Uniforms: iResolution, iTime, iMouse, iChannel0 (children).", sub("code")),
      prop("uniforms", "Uniforms", "json", null, "content", "Extra uniform values by name ({\"uStrength\": 0.5}).", { advanced: true }),
      ...CORNER_PROPS,
    ],
