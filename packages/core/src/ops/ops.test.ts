...
describe("addPatch", () => {
  it("fills default typeParam and inputCount and places patches left to right", () => {
    ...
    expect(main(r.doc).patches).toStrictEqual({
      ...
      sum: { type: "add", name: "Sum", typeParam: "number", inputCount: 2, inputs: {}, ui: { x: 310, y: 40 } },
...
describe("updatePatch", () => {
  it("prunes values that no longer fit after a type change and undo restores them", () => {
    ...
    const c = main(r.doc);
    expect(c.patches.grow).toStrictEqual({ type: "transition", typeParam: "number", inputs: { progress: { link: "pop.output" } }, ui: { x: 305, y: 40 } });
