...
import type { Component, Diagnostic, Id, KnobSet, LayerNode, Op, PatchNode, PatchSpec, Registry, Severity, SonobeDocument, SonobeError, Suggestion, ValueType } from "./types.ts";
...
import { formatNumber, isAssetInput, isLayerInput, isLinkInput, isLiteral, zeroLiteral } from "./values.ts";
...
function checkComponent(doc: SonobeDocument, c: Component, registry: Registry): ComponentResult {
  ...
  return { component: c, ids, layers, patches, graph, touch: check.touch(), refs, hasFeedback: graph.some((d) => d.code === "feedback_loop"), knobReads: componentKnobReads(c) };
}
...
function updateComponent(doc: SonobeDocument, c: Component, prev: ComponentResult, registry: Registry): ComponentResult | null {
  ...
  return { component: c, ids: prev.ids, layers, patches, graph, touch, refs: prev.refs, hasFeedback: graph.some((d) => d.code === "feedback_loop"), knobReads: prev.knobReads };
}
