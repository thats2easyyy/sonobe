...
- **Leave layout to the tools.** Leave out `ui`: `add_patches` places new patches as wide as the editor draws them, and `tidy_graph` lays out a comment frame and reports overlaps (the `graph-basics` guide).
- **Their viewer keeps state.** Your edits reach the person's live prototype without restarting it, so a count or a switch that's on stays. `restart_viewer` starts it over, on their phone too.
