...
- `updateInterface` merges ports by key: a port you name replaces that key, `null` unpublishes it, and other keys stay. `"replace": true` makes each side you send the whole set. Unpublishing disconnects the port's cables inside and on every instance, and so does declaring a port again with a type they no longer fit; the result lists them, and undo brings them back. To rename a key, unpublish it and publish the new one. A layer component's inputs can't reuse a property every instance has (`position`, `opacity`, `enabled`…).
