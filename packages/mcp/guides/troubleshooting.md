...
- `loops_inside_single_copy` (info): a layer you drag or tap makes 1 copy while looped layers inside it repeat on their own, stacked in that one copy. Apply the op: it links the loop to the layer's `repeat`.
- `loop_length_mismatch` (warning, or info when it looks deliberate): loops of different lengths meet at a patch or at a layer's copies, so the shorter one starts over (or items past the last copy don't show). Give the loops the same length, or link `repeat` to the loop the copies should follow. The document check covers lengths it can see; sim results report the rest at runtime.
- `repeat_inside_repeat`, `repeat_from_own_gesture` (warnings): a `repeat` under a layer that already makes copies is ignored, and one that follows a gesture on its own copies can stick at 0 or 1. Link `repeat` to the data.
...
- **Only the text repeats, stacked inside one card**, and a drag moves them all: the loop reaches the card's children, not the card. Link the loop to the card's `repeat` (see `loops`).
...
- **A headless screenshot looks slightly off.** Headless servers draw the screen themselves: text uses approximate metrics, and video, Lottie and shaders are placeholders. Graph and canvas screenshots are drawn from the document too, with estimated node sizes. Check exact values with `sim_get_values`, or open the project in the Sonobe app.
- **`reveal` says "not revealed" for items inside a component.** The person is viewing another component, and without `focus` reveal never moves them. Pass `focus: true` to open it for them, or look yourself with `get_screenshot({ "target": "graph", "component": "…" })`. Name items inside instances with a path: `card_1/tap_photo`.
...
- **A call failed with `editor_reloaded`.** The window's editor crashed or reloaded before it answered, so the call may not have finished. The window loads its editor again by itself, and your next call waits for it. Bring back unsaved work from its draft as above, then check the document before retrying.
