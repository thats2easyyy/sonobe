...
- `resolveGraphGeometry(host, snapshot, componentId)` (`src/geometry.ts`): every graph node's box as the editor draws it, estimated and cached per revision, with the sizes an open patch editor measured when the host has them (`SonobeHost.graphGeometry`). `tidy_graph`, `add_patches` and `get_items` use it.
- `drawComponentGraph(doc, registry, componentId, geometry, { scale, maxWidth })` and `designScene(doc, registry, componentId)` (`src/componentViews.ts`): a component's patch graph as SVG and its canvas at frame 0, for `get_screenshot` with `component` when no editor shows it.
...
- **Writes** return `revision`, `txnId`, created ids, `idMap` and diagnostics `{ added, resolved, totals }`. `preview_design` changes no document, so it returns only the document's `revision`, plus the draft's `name`, `component` and `replace` as its calls left them and `draftRevision`, which counts its draft's updates.
...
- `loadExamples` / `defaultExamples`: the examples catalog behind `list_examples` and `get_example` (pass one as `examples` to `createSonobeMcpServer`); `copyExampleTexts` copies the READMEs, tests, project files and captures it reads next to a bundle.
