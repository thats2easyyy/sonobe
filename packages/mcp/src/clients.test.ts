...
describe("client registry", () => {
  it("lists a relay session from its hello, then drops it after it stops heartbeating", () => {
    const r = registry();
    r.clients.hello({ id: A, name: "claude-code", title: "Claude Code", version: "2.1.278", folder: "/Users/me/noddit", relay: { version: "0.1.0" } });
    expect(r.clients.list()).toMatchObject([{ id: A, label: "Claude Code", version: "2.1.278", folder: "/Users/me/noddit", via: "relay", state: "connected", toolCalls: 0, lastActivityAt: null, relayVersion: "0.1.0" }]);
    ...
    expect(r.changes()).toBe(1);
    expect(r.clients.get(A)).toMatchObject({ state: "connected", folder: "/Users/me/noddit" });
...
describe("parseHello", () => {
  it("accepts what the relay sends", () => {
    expect(parseHello({ id: A, name: "claude-code", title: "Claude Code", version: "2.1.278", folder: "/Users/me/noddit", relay: { version: "0.1.0" }, extra: true })).toEqual({
      id: A,
      name: "claude-code",
      title: "Claude Code",
      version: "2.1.278",
