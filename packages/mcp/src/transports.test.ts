...
describe("createHttpHandler: sessions", () => {
  ...
  it("counts tool calls per relay session, and puts clients without the relay in one row", async () => {
    clients.hello({ id: A, name: "claude-code", title: "Claude Code", folder: "/Users/me/placemark" });
    await callAs(A, "list_documents");
    await callAs(A, "get_outline");
    await callAs(null, "list_documents");
    expect(clients.get(A)).toMatchObject({ label: "Claude Code", folder: "/Users/me/placemark", toolCalls: 2, lastTool: "get_outline", state: "connected" });
  ...
  it("keeps each session's working badge, so one finish_work doesn't clear the other", async () => {
    ...
    expect((await sessions.host.presence()).map((w) => [w.intent, w.client?.folder])).toEqual([
      ["Tuning the deck", "/Users/me/placemark"],
      ["Adding a tab bar", "/Users/me/sonobe"],
    ]);
    expect(await callAs(A, "get_document_info")).toContain("Working: Claude (Claude Code in /Users/me/placemark) — Tuning the deck; Claude (Claude Code in /Users/me/sonobe) — Adding a tab bar");
