...
export function serverInstructions(host: SonobeHost): string {
  const lines = [
    ...
    ...(host.capabilities.designPreview ? ["   To design a new screen, show it on the canvas as you write with preview_design. Start it within your first few steps with the page's head, theme and first section instead of planning the whole page first, append section by section, then import it with import_design (preview: true)."] : []),
