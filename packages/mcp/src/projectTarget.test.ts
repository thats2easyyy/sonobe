...
async function project(dir: string) {
  ...
  await writeFile(path.join(dir, "project.json"), '{ "formatVersion": 1, "name": "Placemark", "root": "main" }\n');
}
...
describe("new project folders", () => {
  ...
  it("refuses a folder inside another project, and says where it could go instead", async () => {
    // The test session's case: the Save panel was inside placemark-test.sonobe.
    await project(path.join(home, "Documents", "Sonobe", "placemark-test.sonobe"));
    const nested = path.join(home, "Documents", "Sonobe", "placemark-test.sonobe", "Untitled.sonobe");
  ...
  it("knows placeholder names", () => {
    ...
    for (const name of ["Placemark Deck", "Untitled Deck", "Photo Zoom"]) expect(isPlaceholderName(name)).toBe(false);
  });
});
