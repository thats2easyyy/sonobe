  ...
  tc.tool(
    ...
    async ({ id, detail, batch }) => {
      ...
      const head = [
        ...
        ...(design ? [`Starts from a design import (detail "design" gives its capture); its recipe wires up the imported layers.`] : []),
      ];
      const data = {
        id: entry.id,
        name: entry.name,
      ...
      if (d === "design") {
        if (!design)
          return failure({
            code: "no_design",
            message: `${entry.name} doesn't start from a design import, so there's no capture to import: its ops make every layer.`,
            hint: `get_example({ "id": "${entry.id}", "detail": "ops" }) gives its recipe.`,
          });
        let capture: unknown;
        try {
          capture = entry.capture === undefined ? undefined : JSON.parse(entry.capture);
        } catch {
          capture = undefined;
        }
        if (capture === undefined)
          return failure({
            code: "example_unavailable",
            message: `The design ${entry.name} starts from (examples/${design.capture}) isn't beside this Sonobe.`,
            hint: "This is a bug in Sonobe; follow the pattern in a design of your own meanwhile.",
          });
        const lines = [
          `# ${entry.name} (${entry.id}): the design it starts from`,
          `The design capture its recipe imports first. On a blank document (create_document), pass this JSON whole as import_design({ "capture": ... }); it makes the layers and images the recipe's ops name, and its photos download from their URLs. Then apply the ops: get_example({ "id": "${entry.id}", "detail": "ops" }).`,
          "",
          "```json",
