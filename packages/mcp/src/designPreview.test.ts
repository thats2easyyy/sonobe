...
describe("preview_design", () => {
  ...
  it("notes a new screen placed entirely outside the device screen, where the canvas and viewer won't show it", async () => {
    ...
    const nameless = await session(withCanvas(project.host), PLACEMARK);
  ...
  it("never draws a call cancelled while it waited for its turn, and leaves the draft as it was", async () => {
    ...
    const placemark = await session(withCanvas(project.host), PLACEMARK, () => signals.shift());
    let release!: () => void;
    slowFor = { key: PLACEMARK, until: new Promise<void>((resolve) => (release = resolve)) };
    const slow = placemark.call("preview_design", { name: "Checkout", html: "<body>checkout" });
    const late = placemark.call("preview_design", { append: "<main>late</main>" });
    ...
    slowFor = null;
    const after = await placemark.call("preview_design", { append: "</body>" });
