...
const NODDIT = "11111111-aaaa-4bbb-8ccc-000000000001";
...
describe("preview_design", () => {
  ...
  it("keeps each session's draft apart", async () => {
    ...
    const noddit = await session(host, NODDIT);
    const sonobe = await session(host, SONOBE);
    await noddit.call("preview_design", { name: "Checkout", html: "<body>checkout" });
    await sonobe.call("preview_design", { name: "Inbox", html: "<body>inbox" });
    await noddit.call("preview_design", { append: "</body>" });
    expect(updates.map((u) => [u.key, u.client?.id, u.name, u.html, u.draftRevision])).toEqual([
      [NODDIT, NODDIT, "Checkout", "<body>checkout", 1],
      [SONOBE, SONOBE, "Inbox", "<body>inbox", 1],
      [NODDIT, NODDIT, "Checkout", "<body>checkout</body>", 2],
    ]);
    ...
    expect((await noddit.call("preview_design", { append: "<p>still here</p>" })).isError).toBe(false);
  });
  ...
  it("doesn't hold one session's draft up behind a window that's slow to show another's", async () => {
    ...
    const next = noddit.call("preview_design", { append: "</body>" });
    release();
  ...
  it("drops a draft left alone for 15 minutes, and clears it from the canvas", async () => {
    ...
    const noddit = await session(host, NODDIT);
    const sonobe = await session(host, SONOBE);
    await noddit.call("preview_design", { name: "Checkout", html: "<body>" });
    clock += DRAFT_IDLE_MS;
    ...
    expect((await noddit.call("preview_design", { append: "</body>" })).structured.error).toMatchObject({ code: "no_draft" });
  });
  ...
  it("keeps the draft in headless mode, and notes a canvas that didn't take it", async () => {
    ...
    const c = await session(withCanvas(project.host), NODDIT);
    canvasFails = true;
...
describe("import_design with preview", () => {
  it("imports the draft's html with its name and replace, shows it being added, then takes it off the canvas", async () => {
    const host = withCanvas(project.host);
    const c = await session(host, NODDIT);
    expect((await c.call("import_design", { capture: CHECKOUT, name: "Checkout" })).isError).toBe(false);
