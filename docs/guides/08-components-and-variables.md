...
A published input keeps the port's current value as its default. Press ⌥P on the port again to unpublish it. Unpublishing a port disconnects its cables on every instance; undo brings them back. With nothing selected inside the component, the Inspector lists its published inputs and outputs, where you can rename them, change an input's default, or unpublish them. Group into Component names the new component right away: type a name and press Return.
