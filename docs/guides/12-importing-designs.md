...
3. Watch Claude write it: the canvas makes room, with the patch editor kept as a strip below, and the page draws over the artboard as the HTML arrives, large enough to read, under the hologram's scanner: a thin veil, pixel rain and a laser sweeping down and back up until the page is complete. When it's done, the preview fades into real layers, named from the page, in one undo step, and the new screen is selected. You've already watched it take shape, so it doesn't build in as a hologram.
