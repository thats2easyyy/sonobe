...
- **Current state (Sept 2026):** the legal page (A) and the Agent SDK note (B) still prohibit third-party claude.ai login unless previously approved. **[VERIFIED]**

**POLICY-DRIFT FLAG:** support article D lists "Third-party apps that authenticate with your Claude subscription through the Agent SDK" as a recognized category. Docs A and B say third-party developers may not offer claude.ai login "unless previously approved." The reconciliation I'd assume: approved partners, or sign-in completing inside Anthropic's own flow in the unmodified binary, may be tolerated. Unapproved open-source apps should not rely on it. **[INFERRED; treat as an open question for Anthropic]**
