...
- Design a new screen on the person's canvas, so they watch it take shape. Start the preview within your first few steps instead of planning the whole page first: `preview_design` with the page's head and theme plus its first section as `html`, then `append` the rest section by section, one visual group per call (header, content, bottom bar), then `import_design` with `"preview": true` to turn the draft into layers. Leave `position` out: new screens go at [0, 0], since the canvas and viewer draw only the device screen.
