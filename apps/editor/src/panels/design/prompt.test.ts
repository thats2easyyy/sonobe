...
describe("claudePrompt", () => {
  it("asks Claude Code for a new screen through import_design", () => {
    expect(claudePrompt({ docName: "Noddit", text: " a checkout with Apple Pay ", context: newScreen, browser: false })).toBe(
      `In my open Sonobe prototype “Noddit”, design a new screen for “Main”: a checkout with Apple Pay
...
Write it as one static HTML page 402 points wide, with data-name on everything I'll wire and the top safe area empty, and import it with import_design (component "main"). Then iterate by importing again with replace.`,
  ...
  it("asks Claude Code to redesign the picked layer with replace", () => {
    expect(claudePrompt({ docName: "Noddit", text: "make it darker", context: redesign, browser: false })).toBe(
      `In my open Sonobe prototype “Noddit”, redesign “Focus Card” (layer focus_card, 354 × 200 in “Main”): make it darker
...
${styles}`,
  ...
  it("asks Claude in the browser for HTML to paste, naming no tools", () => {
    const prompt = claudePrompt({ docName: "Noddit", text: "a profile screen", context: newScreen, browser: true });
    expect(prompt).toBe(
      `In my open Sonobe prototype “Noddit”, design a new screen for “Main”: a profile screen
...
Write it as one complete HTML file 402 points wide, with data-name on everything I'll wire and the top safe area empty, that I can paste into Sonobe's File → Import Design → Paste HTML.`,
    ...
    const redesignPrompt = claudePrompt({ docName: "Noddit", text: "make it darker", context: redesign, browser: true });
...
Write it as one complete HTML file 354 points wide, with data-name on everything I'll wire and the top safe area empty, that I can paste into Sonobe's File → Import Design → Paste HTML.`,
  ...
  it("leaves out the styles when the context has none", () => {
    ...
    expect(claudePrompt({ docName: "Noddit", text: "a checkout", context: bare, browser: false })).toBe(
      `In my open Sonobe prototype “Noddit”, design a new screen for “Main”: a checkout
...
Write it as one static HTML page 402 points wide, with data-name on everything I'll wire and the top safe area empty, and import it with import_design (component "main"). Then iterate by importing again with replace.`,
    ...
    expect(claudePrompt({ docName: "Noddit", text: "make it darker", context: bareRedesign, browser: true })).not.toContain("styles");
  });
});
