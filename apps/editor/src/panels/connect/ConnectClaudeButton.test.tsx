...
describe("ConnectClaudeButton", () => {
  ...
  it("doesn't count sessions that left", async () => {
    const button = await show({ ...RUNNING, clients: [session("11111111-aaaa-4bbb-8ccc-000000000001", "/Users/me/noddit", "gone")] });
    expect(button.dataset.state).toBe("ready");
  });

  it("goes green for a connected session", async () => {
    const button = await show({ ...RUNNING, clients: [session("11111111-aaaa-4bbb-8ccc-000000000001", "/Users/me/noddit")] });
  ...
  it("counts several connected sessions", async () => {
    const button = await show({ ...RUNNING, clients: [session("11111111-aaaa-4bbb-8ccc-000000000001", "/Users/me/noddit"), session("22222222-aaaa-4bbb-8ccc-000000000002", "/Users/me/sonobe")] });
    expect(button.dataset.state).toBe("connected");
  ...
  it("shows working while an agent has a badge", async () => {
    editor.presence.getState().begin({ intent: "Tuning the deck", client: { id: "11111111-aaaa-4bbb-8ccc-000000000001", label: "Claude Code", folder: "/Users/me/noddit" } });
    const button = await show({ ...RUNNING, clients: [] });
