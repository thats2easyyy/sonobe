...
export interface LearnDrawerProps {
  ...
  view?: LearnView;
  /** First view when `view` isn't set. Default: the last view this viewer had open, else home. */
...
/**
 * The Learn drawer: the guides from docs/guides with a level map, a guide reader, and the patch
 * reference, with "Try it" for bundled examples. Fills its container; mount inside an EditorProvider.
 */
export function LearnDrawer({ onClose, view, defaultView, onViewChange, onConnectClaude, onInsertPatch, className }: LearnDrawerProps) {
  ...
  const [stack, setStack] = useState<LearnView[]>(() => [view ?? defaultView ?? readLearnView() ?? { kind: "home" }]);
  ...
  const back = () => setStack((s) => (s.length > 1 ? s.slice(0, -1) : [{ kind: "home" }]));
  ...
  const section = current.kind === "patches" ? "patches" : "guides";

  return (
    <div className={cx("sb-learnx", className)}>
      <header className="sb-learnx__header">
        ...
        <SegmentedControl<"guides" | "patches">
          size="sm"
          aria-label="Learn section"
          className="sb-learnx__sections"
          value={section}
          onChange={(next) => navigate(next === "guides" ? { kind: "home" } : { kind: "patches", type: null })}
          options={[
      ...
      <div className="sb-learnx__body sb-scroll" ref={bodyRef}>
        {current.kind === "patches" ? (
          ...
          <GuideReader guide={guide} catalog={catalog} examples={examples} onOpenGuide={openGuide} onHome={() => navigate({ kind: "home" })} onTryExample={(e) => void tryExample(e)} {...(onConnectClaude ? { onConnectClaude } : {})} />
        ) : (
          <GuideHome catalog={catalog} opened={opened} examples={examples} onOpenGuide={openGuide} onOpenPatches={() => navigate({ kind: "patches", type: null })} onTryExample={(e) => void tryExample(e)} />
        )}
      </div>
    </div>
  );
