...
describe("the Assistant's tool endpoint", () => {
  ...
  it("hands each call to the chat's handler with Claude Code's tool_use id, and returns its result", async () => {
    const importResult: ToolCallResult = {
      ...
      meta: { [IMPORT_META_KEY]: { docId: "placemark", screenId: "checkout" } },
    };
    ...
    expect(imported).toMatchObject({ content: [{ type: "text", text: "Imported “Checkout”" }, { type: "image", data: "iVBORw0KGgo=", mimeType: "image/png" }], _meta: { [IMPORT_META_KEY]: { docId: "placemark", screenId: "checkout" } } });
