...
function harness(behavior: Partial<Behavior> = {}, options: Partial<SubscriptionAgentOptions> = {}) {
  ...
  const agent = createSubscriptionAgent({
    ...
    platform: "darwin",
    home: "/Users/test",
    documentFor: async () => ({ docId: "placemark", projectPath: "/Users/test/Placemark.sonobe" }),
...
describe("subscription engine: tools", () => {
  ...
  it("pins a call to the window's document and a design from the box to its component", async () => {
    ...
    expect(h.bridge.calls).toEqual([
      { name: "preview_design", args: { name: "Checkout", html: "<main>", docId: "placemark", component: "main" } },
      // The draft already has its component.
      { name: "import_design", args: { preview: true, docId: "placemark" } },
    ]);
  });

  it("keeps each window's chat in its own window's prototype", async () => {
    const shows: Record<string, string> = { w1: "placemark", w2: "onboarding" };
    ...
    expect(h.agents).toHaveLength(1);
    expect(h.bridge.calls).toEqual(expect.arrayContaining([{ name: "preview_design", args: { name: "Checkout", html: "<main>", docId: "placemark" } }, { name: "preview_design", args: { name: "Checkout", html: "<main>", docId: "onboarding" } }]));
    ...
    expect(results.get("w1")).toEqual([expect.objectContaining({ structuredContent: { docId: "placemark" } }), expect.objectContaining({ isError: true, content: [{ type: "text", text: refusal("onboarding") }] })]);
    expect(results.get("w2")).toEqual([expect.objectContaining({ structuredContent: { docId: "onboarding" } }), expect.objectContaining({ isError: true, content: [{ type: "text", text: refusal("placemark") }] })]);
  });
...
describe("subscription engine: permission cards", () => {
  ...
  it("asks with the agent's choices, and sends the one the person picked back", async () => {
    ...
    expect(h.bridge.calls).toEqual([{ name: "save_document", args: { path: "/Users/test/Documents/Checkout.sonobe", docId: "placemark" } }]);
  ...
  it("says a forced save writes over outside changes, and asks before every one, whatever was allowed for this chat", async () => {
    ...
    for (const [first, allowAlways] of [[{ ask: true }, "allow-with-updates"], [{}, "sonobe-allow-always"]] as const) {
      ...
      expect(h.bridge.calls.map((c) => c.args)).toEqual([{ docId: "placemark" }, { docId: "placemark" }, { force: true, docId: "placemark" }]);
    }
  });
