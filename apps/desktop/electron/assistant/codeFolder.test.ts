...
describe("code folder store", () => {
  it("links a folder for the window and shows home as ~", async () => {
    ...
    expect(await store.link(windowKey, app)).toEqual({ linked: { name: "noddit", path: "~/code/noddit", persisted: false }, missing: false });
    expect(await store.get(windowKey)).toMatchObject({ name: "noddit", persisted: false });
  ...
  it("refuses /, home, a folder that contains home, and things that aren't folders", async () => {
    ...
    await put("README.md", "# noddit");
    for (const folder of [path.join(app, "README.md"), path.join(app, "nope"), "code/noddit", ""]) {
      const err = await linkError(store, folder);
    ...
    expect((await store.link(windowKey, `${app}${path.sep}`)).linked?.name).toBe("noddit");
  });
  ...
  it("reads as missing once the folder is replaced or gone", async () => {
    ...
    await mkdir(app);
    expect(await store.status(windowKey)).toEqual({ linked: { name: "noddit", path: "~/code/noddit", persisted: false }, missing: true });
    const result = await tools.call("list_code_files", {}, scope());
    expect(result.isError).toBe(true);
    expect(text(result)).toBe("The linked folder “noddit” isn't where it was linked (~/code/noddit). Ask the person to link it again.");
  ...
  it("keeps project links in a 0600 file across stores, and window links only in memory", async () => {
    ...
    expect(await store.link({ projectPath: project, windowId: "1" }, app)).toEqual({ linked: { name: "noddit", path: "~/code/noddit", persisted: true }, missing: false });
    ...
    expect(saved.links[project]).toEqual({ root: expect.stringMatching(/noddit$/), name: "noddit", dev: expect.any(Number), ino: expect.any(Number), linkedAt: 1234 });
    if (posix) expect((await stat(file)).mode & 0o777).toBe(0o600);

    const next = createCodeFolderStore({ file, home });
    expect(await next.get({ projectPath: project, windowId: "9" })).toMatchObject({ name: "noddit", persisted: true, linkedAt: 1234 });
  ...
  it("keeps a window's link with the project once the prototype is saved", async () => {
    ...
    await store.link(windowKey, app);
    const project = path.join(home, "Prototypes", "Checkout");
    expect(await store.status({ projectPath: project, windowId: "1" })).toEqual({ linked: { name: "noddit", path: "~/code/noddit", persisted: true }, missing: false });
    expect(await createCodeFolderStore({ file, home }).get({ projectPath: project, windowId: "7" })).toMatchObject({ name: "noddit" });
  });
...
describe("code tools", () => {
  ...
  it("keeps paths inside the folder: ../, absolute paths, NUL, and a sibling folder with the same prefix", async () => {
    ...
    expect(await refused("../noddit-evil/theme.ts")).toContain("is outside the linked folder");
    expect(await refused("~/code/noddit/x")).toContain("is outside the linked folder");
    ...
    expect(text(await tools.call("list_code_files", { path: "../noddit-evil" }, scope()))).toContain("is outside the linked folder");
  });
