...
describe("the Claude subscription (experimental)", () => {
  ...
  it("gives the subscription's engine preview_design, and hides it from the API key's", async () => {
    ...
    await host.createDocument({ path: path.join(dir, "Placemark.sonobe"), template: "photo-zoom" });
  ...
  it("hands the subscription's engine each window's own document, and the readers it needs", async () => {
    ...
    const sub = fakeSubscription();
    const shows: Record<number, { docId: string; projectPath: string | null }> = { 1: { docId: "placemark", projectPath: "/Users/test/Placemark.sonobe" }, 2: { docId: "untitled", projectPath: null } };
    const folders = fakeFolderStore();
    await folders.link({ projectPath: "/Users/test/Placemark.sonobe", windowId: "1" }, "/Users/test/code/placemark");
    ...
    try {
      ...
      expect(await options.codeFolderName?.("1")).toBe("placemark");
