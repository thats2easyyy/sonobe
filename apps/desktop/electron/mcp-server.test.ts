...
describe("startMcpServer", () => {
  ...
  it("takes the relay's hello and goodbye on /clients, behind the token", async () => {
    ...
    const hello = JSON.stringify({ id, name: "claude-code", title: "Claude Code", folder: "/Users/me/noddit" });
    ...
    expect(clients.get(id)).toMatchObject({ label: "Claude Code", folder: "/Users/me/noddit", state: "connected" });

    const relative = await send(s.port, { method: "POST", path: "/clients", headers: auth, body: JSON.stringify({ id, folder: "noddit" }) });
