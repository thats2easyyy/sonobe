...
test.describe("welcome screen", () => {
  ...
  test("opens the Placemark Deck with its photos and knobs", async ({ page }) => {
    const problems = collectConsoleProblems(page);
    await openEditor(page, { welcome: true });
    const welcome = page.getByRole("dialog", { name: "Welcome to Sonobe" });
    const deck = welcome.locator(".sb-template").filter({ hasText: "Placemark Deck" });
    await deck.scrollIntoViewIfNeeded();
    await deck.click();
    await expect(welcome).toBeHidden();
    await expect.poll(() => hook(page, (s) => s.doc().project.name)).toBe("Placemark Deck");
    ...
    await screenshot(page, "welcome-04-placemark-deck");
    expect(problems).toEqual([]);
  });
