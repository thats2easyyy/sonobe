...
Sixteen runnable prototypes, from a single tap to a deck of places built from an imported design. Each folder is a Sonobe project you can open in the app (Learn → Examples) or read with the CLI, and Claude reads them through the MCP tools `list_examples` and `get_example`. Each one comes with a step-by-step README and a `test.json` that simulates the interaction and checks the numbers.
...
| 16 | [Noddit Deck](16-noddit-deck/) | A real deck from an imported design: stacking copies, reading the card above, knobs with a locked reference | Loop Builder, Loop Select, Delay One Frame, Swipe, Spring Animation |
...
├── …
├── 16-noddit-deck/        also knobs.json, and design/: the HTML, capture and photos it's imported from
