...
import { nodditDeck } from "../recipes/16-noddit-deck.ts";
...
describe("buildRecipe", () => {
  ...
  it("points recipes with a design import to buildRecipe", () => {
    expect(() => buildRecipeDocument(nodditDeck, registry)).toThrow(RecipeBuildError);
    expect(() => buildRecipeDocument(nodditDeck, registry)).toThrow(/imports a design.*await buildRecipe/);
  });

  it("imports the design first and keeps its asset files", async () => {
    const built = await buildRecipe(nodditDeck, registry);
