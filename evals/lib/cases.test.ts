...
describe("start documents", () => {
  ...
  it("imports the design first for an example that starts from one", async () => {
    const c = loadCase(
      writeCase("noddit-layers", {
        "case.json": { title: "X", start: { example: "16-noddit-deck", patches: false } },
        "prompt.md": "Hi",
        "test.json": TEST,
      }),
    );
    ...
    expect(applyOps(doc, exampleSolutionOps("16-noddit-deck"), { registry }).ok).toBe(true);
  });
});
...
describe("writeStartProject", () => {
  ...
  it("copies the asset files of the start project or example next to the document", async () => {
    ...
    const starts = [
      ...
      { example: "16-noddit-deck" },
      { example: "16-noddit-deck", patches: false },
    ];
