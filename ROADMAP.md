...
- [ ] Steady nodes: a running prototype doesn't resize patch nodes. Live values sit in slots as wide as the longest text their type prints, there before the first value and whichever loop copy is watched, ending in "…" past it and short of cutting a label; the header's "×N" badge keeps two digits' room; and knob chips keep the room of their slider's or fields' longest value, in the patch editor and in the headless size model alike (partial: a loop passing 99 copies still widens its badge and live values by a digit, a json or any output still widens its node when its first value arrives, and a knob value typed or fine-tuned with ⌥ past its slider's step still widens its chip)
