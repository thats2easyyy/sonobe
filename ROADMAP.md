...
- [ ] Steady nodes: a running prototype doesn't resize patch nodes. Live values keep the room of the longest text their type prints, short of cutting a label, and knob chips the room of their slider's longest value, in the patch editor and in the headless size model alike (partial: a loop past 99 items, a point past ±999.9, a number past a quadrillion, a knob value typed or fine-tuned with ⌥ past its slider's step, and the header's "×N" loop badge gaining a digit on a node its header sets the width of still widen the node)
