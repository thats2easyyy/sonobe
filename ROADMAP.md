...
- [ ] The Assistant on your Claude subscription (experimental): with a switch in Settings → Claude, the Assistant and its Design with Claude box run Claude through Claude's agent adapter (`@agentclientprotocol/claude-agent-acp`, over ACP) with the Claude login already on the computer, and no API key. Replies stream with their tool steps, Stop works, and Claude Code's permission questions (before saving, opening or creating a prototype) show in the chat. Sonobe's tools reach the adapter over a per-chat loopback endpoint and the same tool runner as the API key, so edits stay in the window's prototype and Claude asks before replacing a hand-edited screen; the canvas draws through `preview_design`. A missing adapter, a signed-out login and a crash each say what to do (partial: built and tested against a fake ACP agent, off by default, and not released until Anthropic agrees; what's left is a check with a real Claude login in the app, Anthropic's permission, and then deciding the default)
