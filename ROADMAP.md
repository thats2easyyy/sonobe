...
- [x] Patch editor arrivals: a graph opens in a wave that follows the signal flow, with cables drawing from their outputs; nodes and cables added later (by the person, by undo, by Claude) animate in once; a slow first fit shows skeleton nodes; large graphs and reduced motion fade
