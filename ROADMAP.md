...
- [ ] Downloads: release builds, a prebuilt `.mcpb`, and a plugin marketplace listing (partial: all three build locally, but nothing is published, and a marketplace copy must ship the plugin's built `dist/`. A release must keep the experimental Claude subscription switch hidden until Anthropic agrees, as packaged builds do unless started with `SONOBE_CLAUDE_SUBSCRIPTION=1`)
