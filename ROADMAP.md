...
- [x] Import hologram: the Import Design dialog shows a hologram scanner while it captures, and every import (the dialog, a pasted capture, Claude's `import_design`) lands on the canvas as a hologram whose laser traces the wireframe div by div, then materializes the design, with a crossfade under reduced motion
