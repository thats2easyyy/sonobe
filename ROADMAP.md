...
- [x] Import hologram: the Import Design dialog shows a hologram scanner while it captures, and every import (the dialog, a pasted capture, Claude's `import_design`, which marks its apply as an import) lands on the canvas as a hologram, except one Design with Claude's live preview drew, which is built in front of you instead (rain and a looping laser from the moment you ask, loader boxes as Claude writes, a final sweep that reveals the page) and fades from the preview onto its layers. The hologram's laser traces the wireframe div by div, then materializes the design and closes with a soft bloom, with the screen's selection held back until it lands and a crossfade under reduced motion. Rounded screens keep their corners. The Viewer's device screen traces the same wireframe on the same timeline, or plans it from the running prototype and plays it alone when no canvas shows the screen
...
- [x] Design with Claude on the canvas: describe a screen or pick a layer, watch the in-app Assistant write it as HTML in a sandboxed live preview over the artboard, and get real layers in one undo step as the preview fades onto them, matched to the prototype's styles and a linked code folder. Follow-ups and wiring happen in the same box, and replacing a screen Claude didn't make, or one you changed since, asks first and names what goes. Without an API key, the box copies a prompt for Claude Code
