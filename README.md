...
- **16 examples** in [examples/](examples/README.md), from Tap to Grow to the Noddit Deck. Each is a project folder with a step-by-step README and scripted tests that simulate the interaction. Open them from the welcome screen or the Learn drawer.
...
- **Design import** from a running app, HTML, or Claude, into real layers, or designed with Claude right on the canvas: Claude Code draws a new screen over the artboard as it writes, and the Design with Claude box can start it in Claude Code for you ([guide 12](docs/guides/12-importing-designs.md)).
...
- **Phone preview and a pop-out viewer.** Scan a QR code to run the live prototype in your phone's browser on the same network, with sound and network requests like the viewer. The phone opens a plain http:// address, and browsers keep the camera, microphone, location and iPhone motion for secure pages, so test the camera and microphone in the desktop viewer or the pop-out window. Restarting in Sonobe restarts it on the phone, and a three-finger tap opens a menu there. On an iPhone, the Sonobe Viewer app plays it with real haptics ([apps/ios](apps/ios/README.md); build it with Xcode).
- **An optional in-app Assistant** in the desktop app, for people who'd rather use their own Anthropic API key than Claude Code or Claude Desktop. The key is kept in your operating system's keychain. Its Design with Claude box draws a new screen on the canvas as Claude writes it, matched to your screens and, if you link it, your code, then keeps prototyping it with you. Experimentally, the Assistant can also run on your Claude subscription through Claude's agent adapter, which you install yourself. That's off by default behind a switch in Settings → Claude, awaiting Anthropic's permission, and only the desktop app run from source (`npm run desktop`) shows the switch. Packaged builds, like the DMG above, never do, and no release will offer it until Anthropic agrees ([guide 11](docs/guides/11-working-with-claude.md#experimental-the-assistant-on-your-claude-subscription)).
