...
- Sonobe never offers a claude.ai login of its own and never reads Claude credentials. (The experimental option's Sign in… opens Claude Code's own login in Terminal, below.)
- This section used to say Sonobe never drives a user's subscription headlessly. That can't stay absolute: behind an experimental switch that is off by default, the in-app assistant can run on the person's Claude subscription through Claude's agent adapter (**The Assistant on a Claude subscription**, below). The switch is labelled as awaiting Anthropic's permission, and the option isn't part of any release until Anthropic agrees.
...
**The Assistant on a Claude subscription** (experimental, `apps/desktop/electron/assistant/acp`). Off by default, and not part of any release until Anthropic agrees.
- **The switch.** Settings → Claude → Experimental has "Use my Claude subscription in the Assistant", off by default and labelled "Experimental · awaiting Anthropic's permission". With it on, the Assistant's setup offers "Claude subscription" beside "API key". Anthropic's support article on [using the Agent SDK with a Claude plan](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan) says: "Claude Agent SDK, claude -p, and third-party app usage still draw from your subscription's usage limits." The main process keeps the switch and the pick (`assistant/connection.ts`, `userData/assistant-connection.json`, 0600, `{ subscriptionEnabled: false, provider: "api_key" }` by default) and enforces them; the renderer only asks (`setConnection`).
...
- **Two engines, one protocol.** The API-key agent (`agent.ts`) and the subscription engine (`acp/engine.ts`) implement one `AssistantAgent`, and `register.ts` picks per chat: a chat runs on the provider that was active at its first message (`chatProvider`) until New chat. Changing what a window runs on starts a new chat in that window only. A subscription chat whose switch was turned off since fails with `subscription_off`.
- **Sonobe's tools, per chat.** A loopback MCP server in the main process (`acp/toolServer.ts`, 127.0.0.1, a random port) serves `POST /c/<chat>/mcp` with a 256-bit bearer token per chat, checked with the app MCP server's Host, Origin and bearer checks, and `session/new` names it as the `sonobe` server. It lists the Assistant's tools (the MCP tools, `preview_design` included, then the code folder's), and each call goes through the shared tool runner with that chat's running reply, so edits are authored "Assistant", pinned to the window's document, and pass the replace guard and the delete confirmation, as on the API key. The call's `_meta` carries Claude Code's tool-use id (`claudecode/toolUseId`), so its progress and result land on the chip the ACP `tool_call` started. A call while the chat has no reply running gets an error result. The token is revoked on New chat and when the window closes.
- **Kept apart from the person's Claude Code setup.** Sessions start with `tools: []` (no built-in tools: Claude sees only Sonobe's), `settingSources: []` (none of their settings, hooks, CLAUDE.md or plugins), `strictMcpConfig: true` (none of their MCP servers), `persistSession: false` (Sonobe's chats don't show up in `claude --resume`), and `ENABLE_TOOL_SEARCH=false`, so no Sonobe tool is deferred. The adapter's environment drops `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN`, so the plan pays rather than a stray key, and the variables of a Claude Code session Sonobe may have been started from (`CLAUDECODE`, `CLAUDE_CODE_ENTRYPOINT`, `CLAUDE_CODE_SSE_PORT`, `CLAUDE_PROJECT_DIR`). It keeps the rest, such as `CLAUDE_CONFIG_DIR` and `ANTHROPIC_BASE_URL`.
- **The system prompt carries the tool guide.** Under a custom system prompt the Agent SDK doesn't pass MCP server instructions to Claude, so Sonobe's prompt (`_meta.systemPrompt`) adds the MCP server's instructions and the design guide's preview variant (`systemPrompt(instructions, { drawing: "preview" })`). The API-key prompt doesn't change.
- **Drawing.** Claude Code doesn't forward a tool's input while Claude writes it, so this path has no `design_draft` events. Claude draws with `preview_design` (the page's head and first section, then one `append` at a time) and imports with `import_design` and `preview: true`, which the canvas draws as it does for MCP clients (§13). The runner keeps each document's draft fields, so the replace guard checks that import with the draft's `replace`.
- **What asks first.** Tools that stay inside the window's prototype run without asking, as on the API key: every change goes through `applyOps`, is labelled "Assistant", and can be undone. `save_document`, `open_document` and `create_document` reach outside it, so they're left out of `allowedTools`, and Claude Code asks through ACP (`session/request_permission`) before calling them. The chat shows a card with the adapter's choices, named Allow, Allow for this chat (that ACP session only) and Don't allow. A question with no reply running, or after Stop, is answered cancelled. Sonobe's own confirmations (big deletes, replacing a screen) stay as they are.
- **Models, limits, Stop.** The same model picker: a session starts on the chosen model, switches with `session/set_config_option` when the adapter offers it, and otherwise keeps its model, with a notice. `maxTurns` goes to the adapter. The token budget isn't enforced, since the plan's own limits apply; tokens are still counted and shown, with no cost. Stop settles the reply's confirmations and tool calls and sends `session/cancel`. A reply that hasn't settled 10 s later ends as stopped, its session is dropped, and the next message opens a new one with a notice that it doesn't remember the chat's earlier messages.
- **Sonobe never reads Claude credentials.** The adapter's Claude Code signs in and keeps the login where it always does. **Sign in…** (macOS) writes a one-time script into `userData/handoff`, as Open in Claude Code does, that runs the adapter's own `--cli auth login --claudeai` in Terminal, and Check again restarts an idle adapter so it sees the new login. Sonobe reads only what the adapter reports in `_auth/status_update` (the kind of login, its label, plan and email), and the setup warns when the adapter would bill an API key or another provider instead of the plan.
- **Errors say what to do.** `agent_not_installed`: install the adapter with npm (it needs Node.js 22 or later). `not_signed_in` (the adapter's auth_required, or a signed-out status): Sign in, or run `claude auth login`. `agent_crashed` (the adapter exits during a reply): its exit code and last stderr line, then send the message again to restart it, and update the adapter if it keeps happening. The restarted session starts with the same notice. `agent_failed`: it wouldn't start, answer `initialize` within 20 s, or open a session. `usage_limit`: the plan's limit is reached (matched from the adapter's text, best effort).
- **Tests** use a fake ACP agent (`apps/desktop/tests/fake-claude-agent.mjs`, through `SONOBE_CLAUDE_AGENT`) that calls the per-chat endpoint the way the adapter does. Nothing in CI starts the real adapter or needs a Claude account.
