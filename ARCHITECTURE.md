...
- **Patch editor** is built on `@xyflow/react` with custom node rendering:
  ...
  - Nodes and cables animate in the first time the canvas shows them (`appear.ts`, one store per editor):
    - A first reveal (opening the editor, entering a component, a replaced document) starts when the viewport shows: after the first fit, or once React Flow has placed a saved view and measured the nodes. Comment frames fade in, the nodes in view materialize in a wave that follows the signal flow left to right (a fade, an 8 px travel and a ring in the category color; over 64 nodes skip the ring), and each cable draws from its output as that node comes in, never ahead of the node at its input. More than 150 nodes, or reduced motion, fade in with the viewport instead. A first fit that keeps the canvas waiting on an idle page for more than 250 ms shows skeleton nodes, which fade as the graph arrives; a load that keeps the page busy until its fit doesn't show them.
    ...
    - Each appearance plays once: remounts from panning (`onlyRenderVisibleElements`), drags, collapses and renames don't replay it. The store styles React Flow's wrappers as they mount and moves a node with `translate` on its wrapper, never a scale, so handle positions measured mid-animation stay right. The animations are CSS, so screenshots taken with animations disabled show the end state, and `graph.bounds` waits for appearances to finish. `e2e/appear.spec.ts` covers a component's second entry at its saved view, option-drag copies and link search.
  ...
  - A glowing orb that travels a cable from the output's dot into the input's when a pulse fires or a boolean turns on (a smaller, fainter one when it turns off), with a tapered comet tail and a flare and ripple on the input's dot, drawn above the nodes; and a glow for true state, which changes with the orb: the cable keeps the glow it had until its orb leaves, and the orb lights or darkens it behind the head. An orb leaving a node waits for the orb flying into it to land, so a chain reads as cause and effect. Orbs are Web Animations replayed on reused elements, so none re-renders React, with keyframes closer together where the cable bends, so the tail stays on the wire. A cable has at most two in flight: pulses inside that gap are dropped, a boolean's latest change is held until it opens (a quick tap shows both on and off), and nothing is mounted while idle. Zoomed far out only the head travels, and with reduced motion (the OS, or Settings → Motion) the cable flashes in place instead.
  - Links:
  ...
  - Organization: **Tidy Up** (⌃T, elkjs), comments (frames), components (⌃⌘G), enter/exit component (double-click / ⌥↑).
    ...
    - A node keeps its width while the prototype runs. An output's live value sits in a slot as wide as the longest text its type prints (`liveReserve`, from `formatValueReserve` in `format.ts`: 8 characters for a number, 6 for a progress, an angle or an index, "Off", a 9-character color, quotes around 10 characters of text, the longest enum option, a point's coordinates out to ±999.9, a loop's "×N" with a 6-character preview of its first item or the watched copy's "#k " and the item, counts keeping two digits; at most 96 pt), right-aligned against the label. A value that prints longer, like a flick's velocity, ends in "…" there, and its hover card has it whole. A row that would pass the node's 320 pt maximum gives its value only the room its labels leave, down to 4 characters (`liveRoom` in `nodeSize.ts`, which `deriveGraph` puts on the port), so labels stay whole. A knob chip's value keeps the room of the longest value its slider or fields reach (`knobValueReserve`), or what the knob's whole name leaves (`knobValueRoom`), and then the chip is as wide as chips get, so a longer value pushes into the name instead of widening the node. `OutputPort` and `KnobChip` set the reserves in `ch` of the mono font plus half a pixel, since N ch lays out a 64th narrower than N glyphs, and `measureNode` sizes the same slots, so the estimate matches with any live value.
...
- **Front doors**: File → Import Design… (URL, HTML, or a Claude prompt), pasting a capture on the canvas, the `import_design` MCP tool (`url`, `html`, or `capture`), and the Chrome extension (`integrations/chrome-extension`: the service worker runs the walker in the tab's main world, embeds cross-origin images when the person allows it, and copies the capture; the element picker marks one element for the walker's `selector`). Each import is one history group. While the dialog captures, a hologram scanner with the import's proportions (pixel rain, a faint grid, a laser sweeping down and back up every 2.75 s) covers the form beside the progress, sharing the form's grid cell so the dialog keeps its size. When an import lands, the canvas builds the new screen as a hologram (`apps/editor/src/panels/import/HologramBuild.tsx`): the design hides under a dark veil with pixel rain, a laser sweeps down (1.6 s for a phone screen) and each layer's wireframe traces in as it passes the layer's top edge (text as line bars, images as a box with an X, corners rounded where a rounded clipping ancestor cuts them), children after their parents and a row's siblings one by one, its sides reaching only as far as the laser has scanned; then the laser sweeps back up (1.3 s), the design materializes beneath it and the wireframes fade, and a thin halo glows out around the frame. Sweeps scale with the screen's height, and the whole build stays near 5 s. The screen's selection outline, handles and size badge wait until the halo. `hologramPlan.ts` schedules it as pure data (the 600 largest visible layers at least 2 pt across, near-equal sizes sampled across the screen; nothing inside a hidden or transparent layer), `hologramDraw.ts` draws both the scanner and the build on one 2D canvas each, from `--holo-*` tokens read once, and a `requestAnimationFrame` loop follows the viewport without React renders. `hologram.ts` holds the requests and the screen being covered: `importCapture` asks for one (the dialog and pasted captures), and `watchAgentImports` asks for one when an agent change's label starts with "imported" or "re-imported" (Claude's `import_design`), for the outermost layer holding most of the change. It never takes a pointer; a click, a key (not a modifier or Space) or an undo ends it with a 150 ms fade, while wheel, Space-drag and middle-button pans keep it playing. Undoing the import or opening another document removes it, and a screen the canvas isn't drawing skips it. Reduced motion (Settings → Motion's `data-motion`, which follows the OS setting unless the person picks Full or Reduced, or `data-reduced-motion`) shows no rain and no sweeps, only a 300 ms crossfade from the wireframe to the design.
