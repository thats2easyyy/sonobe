...
- **Patch editor** is built on `@xyflow/react` with custom node rendering:
  ...
  - Nodes and cables animate in the first time the canvas shows them (`appear.ts`, one store per editor):
    - A first reveal (opening the editor, entering a component, a replaced document) starts when the first fit shows the viewport. Comment frames fade in, the nodes in view materialize in a wave that follows the signal flow left to right (a fade, an 8 px travel and a ring in the category color), and each cable draws from its output once both ends are in. More than 150 nodes, or reduced motion, fade in with the viewport instead. A first fit slower than 250 ms shows skeleton nodes, which fade as the graph arrives.
    - Later, a node the canvas isn't showing (inserted, pasted, duplicated, brought back by undo, added by Claude) animates in at once, several in one short wave. New cables draw in too, except one the person connected between nodes already on the canvas.
    - Each appearance plays once: remounts from panning (`onlyRenderVisibleElements`), drags, collapses and renames don't replay it. The store styles React Flow's wrappers as they mount and moves a node with `translate` on its wrapper, never a scale, so handle positions measured mid-animation stay right. The animations are CSS, so screenshots taken with animations disabled show the end state, and `graph.bounds` waits for appearances to finish.
