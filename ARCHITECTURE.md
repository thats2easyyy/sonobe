...
- **Front doors**: File → Import Design… (URL, HTML, or a Claude prompt), pasting a capture on the canvas, the `import_design` MCP tool (`url`, `html`, or `capture`), and the Chrome extension (`integrations/chrome-extension`: the service worker runs the walker in the tab's main world, embeds cross-origin images when the person allows it, and copies the capture; the element picker marks one element for the walker's `selector`). Each import is one history group. While the dialog captures, a hologram scanner with the import's proportions (pixel rain, a faint grid, a laser sweeping down and back up every 2.75 s) covers the form beside the progress, sharing the form's grid cell so the dialog keeps its size. When an import lands, the canvas builds the new screen as a hologram (`apps/editor/src/panels/import/HologramBuild.tsx`): the design hides under a dark veil with pixel rain, a laser sweeps down (1.6 s for a phone screen) and each layer's wireframe traces in as it passes the layer's top edge (text as line bars, images as a box with an X), children after their parents and a row's siblings one by one; then the laser sweeps back up (1.3 s), the design materializes beneath it and the wireframes fade, and the frame glows out. Sweeps scale with the screen's height, and the whole build stays near 5 s. `hologramPlan.ts` schedules it as pure data (the 600 largest visible layers at least 2 pt across), `hologramDraw.ts` draws both the scanner and the build on one 2D canvas each, from `--holo-*` tokens read once, and a `requestAnimationFrame` loop follows the viewport without React renders. `hologram.ts` holds the requests: `importCapture` asks for one (the dialog and pasted captures), and `watchAgentImports` asks for one when an agent change's label starts with "imported" or "re-imported" (Claude's `import_design`), for the outermost layer holding most of the change. It never takes a pointer; a click, a key (not a modifier or Space, which zoom and pan) or an undo ends it with a 150 ms fade, undoing the import or opening another document removes it, and a screen the canvas isn't drawing skips it. Reduced motion (the OS setting, Settings → Motion, or `data-reduced-motion`) shows no rain and no sweeps, only a 300 ms crossfade from the wireframe to the design.
