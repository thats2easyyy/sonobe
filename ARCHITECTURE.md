...
- **Patch editor** is built on `@xyflow/react` with custom node rendering:
  ...
  - Organization: **Tidy Up** (⌃T, elkjs), comments (frames), components (⌃⌘G), enter/exit component (double-click / ⌥↑).
    ...
    - A node keeps its width while the prototype runs. An output's live value sits in a slot as wide as the longest text its type can print (`liveReserve`, from `formatValueReserve` in `format.ts`: 8 characters for a number or index, "Off", a 9-character color, quotes around 10 characters of text, the longest enum option, a point's coordinates out to ±999.9, a loop's "×N" or the watched copy's "#k " with two digits around one item; at most 96 pt), right-aligned against the label. A row that would pass the node's 320 pt maximum keeps only the room its labels leave (`liveRoom` in `nodeSize.ts`, which `deriveGraph` puts on the port), so labels stay whole; only a value longer than that room cuts them, as it would without a reserve. A knob chip's value keeps room for the longest value its slider snaps to (`knobValueReserve`). `OutputPort` and `KnobChip` set the reserve as a `min-width` in `ch` of the mono font, and `measureNode` sizes the same slots, so the estimate matches with any live value.
