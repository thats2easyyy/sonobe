...
- `@sonobe/renderer/svg` draws static SVGs without a DOM: a `SceneFrame` (`sceneToSvg`, headless screenshots) and a patch graph model with node boxes (`graphToSvg`, graph screenshots of a component nobody has open, in the patch editor's dark theme).
- **Platform services for live prototypes** (`platform.ts`): `createBrowserPlatform` gives the runtime network requests, links, speech, keyed audio voices, WebSockets, location, gamepads, device motion (asking iOS for permission inside the next tap), the soft keyboard, picking and reading media, and camera and microphone capture, behind one mute switch (`getMuteStore`: `SONOBE_MUTE`, `?mute=1`, the desktop host's muted flag, or an automated browser). Blocked sounds retry on the next gesture, which for a touch is pointerup. `createLiveVideoOverlays` (`liveMedia.ts`) shows live camera feeds inside video layers. The editor's viewer and the web player (§9.2) share both; simulations never get them.
...
- **Patch editor** is built on `@xyflow/react` with custom node rendering:
  ...
  - Organization: **Tidy Up** (⌃T, elkjs), comments (frames), components (⌃⌘G), enter/exit component (double-click / ⌥↑).
    ...
    - An input or layer property that reads `$knob.<id>` has no cable: `deriveGraph` gives its port a `knob` chip (the knob's name and running value), and the shape model sizes the chip.
- **Layer ↔ patch bridges:**
...
- **Viewer:** live prototype, device picker, restart ⌘R, frame toggle, 1:1, "show hit targets", and pop-out window. Also serves a LAN web player (QR code), which the Sonobe Viewer iPhone app opens with native haptics (§9.2). Restarting restarts the phones and the pop-out viewer too. While the prototype has an `empty_loop` warning, a notice above it names what has no copies, and Why? opens the warning in Diagnostics.
...
- **The page's CSP** allows code only from the player's own origin, no framing, and no form posts. It lets prototypes reach other hosts the way the editor's viewer can: `connect-src 'self' http: https: ws: wss:` (Network Request, JSON File, scripts, WebSocket Connection) and `img-src`/`media-src` with `data: blob: http: https:` (the images, videos and sounds those return, and Base64 Decode's data URLs). The pop-out window's links still go to the system browser.
