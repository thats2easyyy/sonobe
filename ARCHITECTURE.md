...
- Sonobe never offers a claude.ai login of its own and never reads or stores Claude credentials. (The experimental option's Sign in… opens Claude Code's own login in Terminal, below.)
- This section used to say Sonobe never drives a user's subscription headlessly. That can't stay absolute: behind an experimental switch that is off by default, the in-app assistant can run on the person's Claude subscription through Claude's agent adapter (**The Assistant on a Claude subscription**, below). The switch is labelled as awaiting Anthropic's permission, and packaged builds, which any release would be, hide it unless started with `SONOBE_CLAUDE_SUBSCRIPTION=1`. That stays so until Anthropic agrees.
...
**The Assistant on a Claude subscription** (experimental, `apps/desktop/electron/assistant/acp`). Off by default, hidden in packaged builds, and not to be released until Anthropic agrees.
...
- **Hidden in packaged builds.** The status's `connection.available` says whether this build offers the switch: a build run from a checkout always does, and a packaged one (a DMG, as any release would be) only when started with `SONOBE_CLAUDE_SUBSCRIPTION=1`. When it doesn't, Settings leaves the switch out, it reads off, main refuses to turn it on, and no subscription reply, check or sign-in starts.
- **Why it waits on Anthropic.** Anthropic's support article on [using the Agent SDK with a Claude plan](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan) says: "For now, nothing has changed: Claude Agent SDK, claude -p, and third-party app usage still draw from your subscription's usage limits." But Claude Code's [legal page](https://code.claude.com/docs/en/legal-and-compliance) says Anthropic "does not permit third-party developers to offer Claude.ai login into their own applications, or to route requests through Free, Pro, or Max plan credentials on behalf of their users", and the Agent SDK docs allow claude.ai login only when "previously approved". Whether this option routes requests through the person's plan on their behalf is the question for Anthropic (`docs/research/claude-byo.md`, 2.6), so it stays off and unreleased until Anthropic agrees.
...
- **Always Claude Code's "default" mode.** Claude Code asks before those tools in its "default" permission mode, but not in modes such as `auto` or `bypassPermissions`. The adapter starts a session in the person's own `permissions.defaultMode`, so Sonobe opens sessions with `allowDangerouslySkipPermissions: false`, and when `session/new` reports another mode, it sets "default" (`session/set_config_option` on the `mode` option, else `session/set_mode`). If that fails or doesn't take, it closes the session and the reply fails with `agent_failed` before anything runs. As a second line, when one of the three tools reaches the endpoint and the person didn't allow that call through ACP (or the tool, for this chat), Sonobe shows the same card itself and runs the call only on Allow. One call is never asked about twice.
...
- **Sonobe never reads or stores Claude credentials.** The adapter's Claude Code signs in and keeps the login where it always does. The adapter inherits Sonobe's environment without the keys above, so a `CLAUDE_CODE_OAUTH_TOKEN` the person set there reaches it, as it would any child process. **Sign in…** (macOS) writes a one-time script into `userData/handoff`, as Open in Claude Code does, that runs the adapter's own `--cli auth login --claudeai` in Terminal. Check again restarts the adapter, so it sees the new login, when no chat has a session on it; otherwise it asks a one-off `claude-agent-acp --cli auth status --json`, so other chats keep going. Sonobe reads only what the adapter reports about the login (its kind, label, plan and email), and the setup warns when the adapter would bill an API key or another provider instead of the plan.
- **Errors say what to do.**
  - `agent_not_installed`: install the adapter with npm (it needs Node.js 22 or later).
  - `not_signed_in` (the adapter's auth_required): "Choose Sign in… (it opens Terminal), or run claude-agent-acp --cli auth login in Terminal, then send your message again" (elsewhere, the command alone). A signed-out status shows the same words in the setup, ending "then choose Check again".
  - `agent_crashed`: the adapter exited during a reply (its exit code and one stderr line, preferably the last that names an error, never Node's version line, a timing line or a stack frame, with keys redacted), or it ended the session while it kept running, as when Claude Code stops ("Claude Code stopped unexpectedly during this reply"). Either way the next message opens a new session, with the notice that it doesn't remember the chat's earlier messages, and the error says to update the adapter if it keeps happening.
  - `agent_failed`: the adapter wouldn't start, answer `initialize` within 20 s, open a session, or take the "default" mode ("Sonobe couldn't set Claude Code to ask before it saves or opens files, so nothing ran").
  - `usage_limit`: the plan's usage limit, matched by the Agent SDK's own usage-limit messages (after a leading "Internal error: ") or the adapter's `billing_error` and `account_on_hold`. The message quotes Claude Code's words, such as when the limit resets, and suggests the API key meanwhile. `rate_limited`: a rate limit that isn't the plan's ("Wait a minute, then send your message again"). These two show no button, since the message says what to do.
