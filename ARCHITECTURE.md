...
- **Models, limits, Stop.** The same model picker: a session starts on the chosen model and switches with `session/set_config_option` to the adapter's option for it (the model's own id, else its family's alias, such as `sonnet`) when the adapter offers one; otherwise it keeps its model, with a notice. Neither the step cap (`maxTurns`) nor the token budget is enforced, since the plan's own limits apply: `maxTurns` isn't sent to the adapter, so a reply runs until Claude finishes; tokens are still counted and shown, with no cost. Stop settles the reply's confirmations and tool calls and sends `session/cancel`. A reply that hasn't settled 10 s later ends as stopped, its session is dropped, and the next message opens a new one with a notice that it doesn't remember the chat's earlier messages.
