...
- **Patch editor** is built on `@xyflow/react` with custom node rendering:
  ...
  - Organization: **Tidy Up** (⌃T, elkjs), comments (frames), components (⌃⌘G), enter/exit component (double-click / ⌥↑).
    ...
    - A node keeps its width while the prototype runs. An output's live value sits in a slot as wide as the longest text its type can print (`liveReserve`, from `formatValueReserve` in `format.ts`: 8 characters for a number or index, "Off", a 9-character color, quotes around 10 characters of text, the longest enum option, a loop's "×N" or the watched copy's "#k " around one item; at most 96 pt), right-aligned against the label. A knob chip's value keeps room for the longest value its slider reaches (`knobValueReserve`). `OutputPort` and `KnobChip` set the reserve as a `min-width` in `ch` of the mono font, and `measureNode` sizes the same slots, so the estimate matches with any live value. At the 320 pt maximum width, labels give way to the slot and stay cut the same way while values change.
